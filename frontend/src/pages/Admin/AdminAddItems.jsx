...
const AdminAddItems = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const processedImages = images.map((image) => {
      return image.src.replace(/^data:image\/\w+;base64,/, "");
    });

    let formData = new FormData();

    if (videoFile) formData.append("video", videoFile);
    processedImages.forEach((image) => formData.append("images", image));
    ...
    } else if (type !== "gallery") formData.append("price", data.price);
    formData.append("hash", customSlug(data.title));

    setIsLoading(true);
    try {
      const response = await userStore.publishProduct(formData, type);
  ...
  return (
    <>
      <Helmet>
        <title>Admin page | Add Items</title>
      </Helmet>
                        ...
                        index <= 4 && (
                          ...
                          >
                            <ImageLoader
                              ...
                              containerStyles={`${!index ? " max-h-[600px]" : "max-h-[83px] sm:max-h-[110px]"}`}
                              className={`w-full h-full object-cover rounded-3xl ${!index ? " max-h-[600px]" : "max-h-[83px] sm:max-h-[110px]"}`}
                              draggable={false}
                            />
                            <button
                      ...
                      <div className="absolute opacity-0 invisible pointer-events-none group-hover:opacity-100 group-hover:visible group-hover:pointer-events-auto transition-[opacity,visibility] duration-500 w-max rounded-3xl z-[3] top-[-110px] right-[5px] h-[90px] bg-[#1a1a1a] flex items-center gap-3 p-1">
                        {images.slice(5).map((image, index) => (
                          <ImageLoader
                            ...
                            alt={image.name}
                            key={index}
                            className="w-full max-w-[82px] max-h-[90px] h-full object-cover rounded-3xl"
                          />
                        ))}
                      </div>
                    </motion.div>
            ...
            >
              ...
              <div className="flex flex-col w-full h-auto gap-3 relative">
                {["Usual", "Auction", "Gallery"].map((category, index) => (
                  ...
                  >
                    <input
                      checked={type === category.toLowerCase()}
                      onChange={() =>
                        setValue("itemType", category.toLowerCase())
                      }
              ...
              <div className="w-full flex items-center gap-4 justify-between relative h-auto">
                ...
                <button
                  type="submit"
                  className={`flex font-main rounded-[1.25rem] w-full h-[44px]  font-[600] items-center justify-center  bg-[#FCCB00] text-[#522700] ${
                    isLoading
